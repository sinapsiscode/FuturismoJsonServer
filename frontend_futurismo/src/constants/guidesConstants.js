...
export const GUIDE_STATUS_VALUES = (() => {
  const config = getGuidesConfig();
  return config.guideStatusValues || [
...
export const DNI_REGEX = (() => {
  const config = getGuidesConfig();
  return config.dniRegex || /^\d{8}$/;
})();

export const EMAIL_REGEX = (() => {
  const config = getGuidesConfig();
  return config.emailRegex || /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
})();

export const PHONE_REGEX = (() => {
  const config = getGuidesConfig();
  return config.phoneRegex || /^9\d{8}$/;
})();

// Guide messages for toast notifications
export const GUIDE_MESSAGES = (() => {
  const config = getGuidesConfig();
  return config.guideMessages || {
